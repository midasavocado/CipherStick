...
const __CONVERSION_API__ = ((root) => {
  ...
  <dict>
      <key>WFWorkflowActionIdentifier</key>
      <string>com.apple.MobileSMS.MessageEntity</string>
      <key>WFWorkflowActionParameters</key>
      <dict>
        <key>AppIntentDescriptor</key>
        <dict>
  ...
  function normalizeAppLookupKey(name) {
    if (!name) return '';
    return String(name)
      .trim()
      .toLowerCase()
  ...
  const BUILTIN_APP_BUNDLE_ENTRIES = [
    ['System Settings', 'com.apple.systempreferences', 'System Settings'],
    ['System Preferences', 'com.apple.systempreferences', 'System Settings'],
    ...
    ['Contacts', 'com.apple.MobileAddressBook', 'Contacts'],
  ...
  const BUILTIN_APP_BUNDLES = new Map();
  BUILTIN_APP_BUNDLE_ENTRIES.forEach(([label, bundleId, friendly]) => {
    const key = normalizeAppLookupKey(label);
    if (!key || !bundleId) return;
    BUILTIN_APP_BUNDLES.set(key, {
      bundleId,
      name: friendly || label
    });
  });
  const APP_LOOKUP_CACHE = new Map();
  ...
  const XML = {
    ...
    str(s) {
      ...
      return `<string>${XML.esc(text)}</string>`;
    },
    dict(obj) {
  ...
  function sanitizeAppNameCandidate(value) {
    if (typeof value !== 'string') return '';
    const trimmed = value.trim();
    if (!trimmed || valueIsPlaceholder(trimmed)) return '';
    return trimmed.replace(/^['"]+|['"]+$/g, '');
  }

  function sanitizeBundleIdCandidate(value) {
    if (typeof value !== 'string') return '';
    const trimmed = value.trim();
    if (!trimmed || valueIsPlaceholder(trimmed)) return '';
    const unquoted = trimmed.replace(/^['"]+|['"]+$/g, '');
    return looksLikeBundleId(unquoted) ? unquoted : '';
  }

  async function fetchBundleIdFromAppStore(query) {
    ...
    try {
      ...
      const first = data && Array.isArray(data.results)
        ? data.results.find((item) => typeof item?.bundleId === 'string')
        : null;
      if (!first) return null;
      return {
        bundleId: first.bundleId,
        name: first.trackName || first.trackCensoredName || searchQuery
      };
    } catch {
      return null;
    }
  ...
  async function resolveAppBundleMeta(appName) {
    const key = normalizeAppLookupKey(appName);
    if (!key) return null;
    if (APP_LOOKUP_CACHE.has(key)) return APP_LOOKUP_CACHE.get(key);
    if (BUILTIN_APP_BUNDLES.has(key)) {
      const builtin = BUILTIN_APP_BUNDLES.get(key);
      const payload = { ...builtin };
      APP_LOOKUP_CACHE.set(key, payload);
      return payload;
    }
    const fetched = await fetchBundleIdFromAppStore(appName);
    if (fetched) {
      APP_LOOKUP_CACHE.set(key, fetched);
      return fetched;
    }
    APP_LOOKUP_CACHE.set(key, null);
    return null;
  }

  async function resolveAppDescriptor(appNameCandidate, fallbackBundleId) {
    const cleanedName = sanitizeAppNameCandidate(appNameCandidate);
    const explicitBundle = sanitizeBundleIdCandidate(fallbackBundleId) || (looksLikeBundleId(cleanedName) ? cleanedName : '');
    if (explicitBundle) {
      const displayName = cleanedName && !looksLikeBundleId(cleanedName)
        ? cleanedName
        : sanitizeAppNameCandidate(appNameCandidate) || explicitBundle;
      return { bundleId: explicitBundle, name: displayName };
    }
    if (!cleanedName) return null;
    const resolved = await resolveAppBundleMeta(cleanedName);
...
${indentXMLBlock(valueNode, 2)}
  ...
  function resolveConditionMode(value) {
    ...
    if (typeof value === 'string') {
      ...
      if (CONDITION_MODE_MAP.has(normalized)) {
        return CONDITION_MODE_MAP.get(normalized);
      }
      return trimmed;
    }
  ...
  function linkReferencesCurrentApp(quickRef) {
    if (!quickRef || quickRef.type !== 'link' || !quickRef.value) return false;
    const metadata = lookupLinkMetadata(quickRef.value);
    if (!metadata?.action) return false;
    const normalized = normalizeName(metadata.action);
    return normalized === 'getcurrentapp' || normalized === 'getcurrentapplication';
  }
  ...
  async function buildSpecialIf(item) {
    ...
    if (conditionParam == null) {
      ...
      const resolvedCondition = resolveConditionMode(conditionParam);
      conditionNode =
        typeof resolvedCondition === 'string'
          ? XML.str(resolvedCondition)
          : XML.int(resolvedCondition);
    }
    ...
    const compareRawString = typeof compareValue === 'string' ? compareValue.trim() : '';
    ...
    let wfAppBlock = '';
    const explicitAppValue =
      params.WFApp ??
      params.App ??
      params.AppInfo ??
      null;
    if (explicitAppValue != null) {
      const explicitNode = ensureAnyNode(explicitAppValue, 'WFApp', null);
      const rendered = typeof explicitNode === 'string' ? explicitNode.trim() : '';
      if (rendered && rendered !== placeholderToken('WFApp')) {
        wfAppBlock = `<key>WFApp</key>\n    ${indentXMLBlock(String(explicitNode), 4)}\n`;
      }
    } else if (linkReferencesCurrentApp(inputQuick) && compareRawString) {
      const bundleHint = sanitizeBundleIdCandidate(
      ...
      const descriptor = await resolveAppDescriptor(compareRawString, bundleHint);
      if (descriptor) {
        const wfAppDict = XML.dict({
          BundleIdentifier: XML.str(descriptor.bundleId),
          Name: XML.str(descriptor.name || compareRawString || descriptor.bundleId)
        });
        wfAppBlock = `<key>WFApp</key>\n    ${indentXMLBlock(wfAppDict, 4)}\n`;
      }
    }
  ...
  async function buildActionsArrayFromJSON(list) {
    ...
    for (const item of list) {
      ...
      const maybeInjectAutoText = (targetKeys = []) => {
        for (const key of targetKeys) {
          const rawValue = mergedParams[key];
          if (typeof rawValue !== 'string') continue;
          const trimmed = rawValue.trim();
          if (!trimmed || isPureQuickTokenString(trimmed)) continue;
          ...
          const textParameters = {
            ...
            WFTextActionText: renderValue(rawValue)
          };
          out.push(XML.dict({
            WFWorkflowActionIdentifier: XML.str('is.workflow.actions.gettext'),
            WFWorkflowActionParameters: XML.dict(textParameters)
          }));
          for (const clearKey of targetKeys) {
            if (clearKey === key) continue;
            if (mergedParams[clearKey] === rawValue) mergedParams[clearKey] = undefined;
          }
          mergedParams[key] = `!link:${autoLabel}`;
          return true;
        }
        return false;
      };
      ...
      if (normalizedKind === 'setvariable') {
        maybeInjectAutoText(['WFInput', 'Input', 'Value', 'value']);
      } else if (normalizedKind === 'variable.append' || normalizedKind === 'appendvariable') {
        maybeInjectAutoText(['WFInput', 'Input']);
      }
  ...
  async function buildActionFromConversions(actionName, params) {
    try {
      ...
      if (normalizedActionName === 'openapp') {
        const nameCandidate =
          sanitizeAppNameCandidate(
            normalizedParams.AppName ??
            params.AppName ??
            normalizedParams.App ??
            params.App ??
        ...
        const bundleHint = sanitizeBundleIdCandidate(
          normalizedParams.BundleIdentifier ??
          params.BundleIdentifier ??
          normalizedParams.AppBundleIdentifier ??
          params.AppBundleIdentifier ??
          normalizedParams.AppBundleId ??
          params.AppBundleId ??
          null
        );
        const descriptor = await resolveAppDescriptor(nameCandidate, bundleHint);
        const fallbackName = nameCandidate || friendlyValue || 'App';
        const fallbackBundle = bundleHint || descriptor?.bundleId || fallbackName;
        normalizedParams.SelectedBundleIdentifier = XML.str((descriptor && descriptor.bundleId) || fallbackBundle);
        normalizedParams.SelectedAppName = XML.str((descriptor && descriptor.name) || fallbackName);
      }
  ...
  async function buildIfBlock(item) {
    ...
    let conditionMode;
    // Try to resolve from string if provided
    if (Object.prototype.hasOwnProperty.call(params, 'WFCondition')) {
      const val = params.WFCondition;
      if (typeof val === 'string' && CONDITION_CODES[val.toLowerCase()] !== undefined) {
        conditionMode = CONDITION_CODES[val.toLowerCase()];
      } else {
        conditionMode = val;
      }
    } else if (Object.prototype.hasOwnProperty.call(params, 'ConditionMode')) {
    ...
    // Auto-wrap UUID strings into WFTextTokenAttachment
    if (typeof inputValue === 'string' && /^[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}$/i.test(inputValue)) {
      inputValue = {
        Value: {
          Type: "ActionOutput",
          OutputUUID: inputValue,
          OutputName: "Result" // Generic name, but links correctly
        },
        WFSerializationType: "WFTextTokenAttachment"
      };
    }

    if (conditionMode === undefined) conditionMode = 4; // Default to 'Is'
    if (conditionString === undefined) conditionString = ''; // Default empty
